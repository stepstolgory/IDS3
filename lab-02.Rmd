...
author: "Team Name: Stepan Mikoyan, Mason Moir, Sean Wei, and Ryan Quigley, Cailean Clubb"
date: "`r Sys.Date()`"
...
plastic_data_all %>% group_by(region) %>% count() %>% arrange(desc(n))

```
_The max value is 43 for Africa_
...
plastic_data_all<- plastic_data_all %>% mutate(total_mismanage_plastic = mismanaged_plastic * population)

select(plastic_data_all)

plastic_data_all %>% group_by(region) %>%summarise(mean_total = mean(total_mismanage_plastic))%>%arrange(desc(mean_total))
